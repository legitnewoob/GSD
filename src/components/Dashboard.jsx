...
export function Dashboard({ config, entries: allEntries }) {
  ...
  // Always daily — no aggregation, no hidden entries
  const activityData = lineData.map((d) => ({ label: d.date, steps: d.steps ?? 0, km: d.km ?? 0 }));
  const activityInterval = range === 7 ? 0 : 4;
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={activityData} margin={{ left: 10, right: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                <XAxis dataKey="label" tick={{ fill: '#94a3b8', fontSize: 11 }} interval={activityInterval} />
